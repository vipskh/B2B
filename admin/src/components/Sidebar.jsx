...
function Sidebar() {
  ...
  const portalLabel = me?.role === "vendor" ? "Seller Center" : "Admin";
      ...
      <div className="flex min-h-full flex-col items-start bg-base-200 is-drawer-close:w-14 is-drawer-open:w-64">
        <div className="p-4 w-full">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold is-drawer-close:hidden">{portalLabel}</span>
          </div>
        </div>
        ...
        <div className="p-4 w-full">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-full bg-primary/20 flex items-center justify-center shrink-0 font-bold">
              {me?.name?.[0]?.toUpperCase() || "?"}
            </div>
            <div className="flex-1 min-w-0 is-drawer-close:hidden">
              <p className="text-sm font-semibold truncate">{me?.name}</p>
              <p className="text-xs opacity-60 truncate">{me?.email}</p>
            </div>
          </div>
        </div>
      </div>
